...
{
    [Parallelizable]
    public class OneWorldDbClientMonoDbCtxTests : N2SqlLocalDbTestBase<OneWorldDbUnitTestDbContext>
    {
        private readonly ILogger _logger = new N2Logger<OneWorldDbClientMonoDbCtxTests>();
                ...
                using (var diInstance = CreateManager())
                {
                        ...
                        IsolationLevel.ReadCommitted))
                    {
                        ...
                        Assert.AreEqual(1, tx.DbContext.Set<SampleTable01>()
                            .Count(e => e.SampleColumn01 == @"Z9999"));
